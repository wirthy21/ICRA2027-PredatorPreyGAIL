...
Time convention
---------------
``lag=1`` always means one ordered source timestep within a clip/rollout.  It
does not imply a shared physical duration between expert and policy data.
...
OPTIONAL_COLUMNS = ("vx", "vy", "fps", "timestamp", "policy_id")
    ...
    def __post_init__(self) -> None:
        ...
        for name in OPTIONAL_COLUMNS:
            if name not in normalized:
                if name in {"vx", "vy", "fps", "timestamp"}:
...
def to_canonical_trajectory(records: Iterable[Mapping[str, Any]]) -> TrajectoryTable:
    ...
    if not rows:
        return TrajectoryTable({
            ...
            **{name: np.asarray([], dtype=float if name in {"vx", "vy", "fps", "timestamp"} else object)
               for name in OPTIONAL_COLUMNS},
        })
    ...
    for name in names:
        default: Any = np.nan if name in {"vx", "vy", "fps", "timestamp"} else ""
    ...
    for name in ("x", "y", "heading", "vx", "vy", "fps", "timestamp"):
        columns[name] = columns[name].astype(float)
...
def generate_policy_rollouts(
    ...
    *,
    ...
    for rollout_id in tqdm(range(n_rollouts), desc=rollout_label, leave=False):
        rollout_seed = int(seed) + rollout_id
        ...
        simulation_kwargs = dict(
            ...
            visualization="off", init_pool=init_pool, experiment=False,
        ...
        for step, metrics in enumerate(metrics_list):
            ...
            for agent in range(n_prey + 1):
                ...
                records.append({
                    "source": "policy", "condition": "unknown", "clip_id": f"rollout_{rollout_id:04d}",
                    ...
                    "vx": vxs[agent], "vy": vys[agent], "fps": np.nan, "timestamp": np.nan,
...
def generate_couzin_expert_rollouts(
    ...
    *,
    ...
    records: list[dict[str, Any]] = []
    for rollout_id in tqdm(range(n_rollouts), desc=f"Couzin expert {n_prey}", leave=False):
        np.random.seed(int(seed) + rollout_id)
        ...
        for step, metrics in enumerate(metrics_list):
            ...
            for agent in range(n_prey + 1):
                ...
                records.append({
                    ...
                    "clip_id": f"rollout_{rollout_id:04d}", "frame": step, "time_step": step,
...
def trajectory_diagnostics(table: TrajectoryTable) -> dict[str, Any]:
    ...
    return {
        ...
        "timestep_status": "FPS retained where available" if len(finite_fps) else "PHYSICAL TIME UNKNOWN",
    }
...
# a response lag is expressed in source steps rather than physical seconds.
...
def detect_approach_events(
    ...
    """Detect close-and-closing runs using a backward consecutive-step trend."""
    ...
    previous = torch.cat((torch.zeros(1, dtype=torch.bool, device=distance.device), condition[:-1]))
    onsets = torch.nonzero(condition & ~previous, as_tuple=False).flatten()
    return (onsets, condition) if return_mask else onsets
...
def compute_reaction_metrics(
    ...
    *,
    ...
    result = {
        ...
        "cascade_fraction": fraction.clone(),
...
def compute_generalization_error(
    ...
    """Separate group-size stability from imitation fidelity at each size."""
...
def trajectory_table_to_metric_segments(
    ...
    for source, clip_id in sorted(set(zip(source_values, clip_values))):
        ...
        temporal_starts = [0] + (np.where(~np.isclose(np.diff(times), 1.0))[0] + 1).tolist()
...
def analyze_tensor_clip(
    ...
    *,
    ...
    closing_speed_raw = compute_closing_speed(raw_distance, lag=closing_speed_lag)
    closing_speed = compute_closing_speed(normalized_distance, lag=closing_speed_lag)
...
def analyze_case(
    ...
    *,
    ...
    for clip_index, clip_cpu in enumerate(frame_clips):
        ...
        static_valid = risk_index >= 0
        static_x = normalized[static_valid]
        for name, values in (("pursuit_alignment", pursuit), ("escape_alignment", escape),
            ...
            curve_x[name].append(static_x); curve_y[name].append(values[static_valid])
        ...
        for segment in temporal_by_clip[clip_index]:
            ...
            closing = compute_closing_speed(segment_distance, lag=closing_speed_lag)
            if len(closing):
        ...
        if len(closing_values):
            valid = (closing_risk >= edges[0]) & (closing_risk <= edges[-1])
            curve_x["closing_speed"].append(closing_risk[valid]); curve_y["closing_speed"].append(closing_values[valid])
        ...
        if len(response_values):
            valid = ((response_risk_values >= edges[0])
                     & (response_risk_values <= edges[-1]))
            curve_x["continuous_predator_response_curve"].append(response_risk_values[valid])
            curve_y["continuous_predator_response_curve"].append(response_values[valid])
...
def compute_all_group_size_comparisons(
    ...
    """Compute 16/32 changes and generalization errors for every complete source quartet."""
...
__all__ = [
    "ExpertDataUnavailable", "TrajectoryTable", "to_canonical_trajectory", "wrap_angle",
    ...
    "detect_approach_events", "compute_reaction_metrics", "compute_polarization_tensor",
