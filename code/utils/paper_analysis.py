"""Focused, self-contained paper analyses for ``3.1 Paper Analysis.ipynb``.

The module retains the frozen trajectory and geometry primitives required to
reproduce the published estimands, then exposes only the three workflows used
by the focused notebook: the 20-step Couzin cohort, biological data versus
biological GAIL, and the two selected simulation time-step grids.
"""
...
def _windows_binary_path(path: Path) -> str:
    """Support deeply nested data paths on Windows."""
...
def load_expert_32prey(
    ...
    *,
    ...
    """Load continuous trajectory records into the canonical representation.
...
def _load_policy_source_modules(
    ...
    local_code_dir = (PROJECT_ROOT / "code").resolve()

    def is_local_code_entry(entry: str) -> bool:
        try:
            return Path(entry or Path.cwd()).resolve() == local_code_dir
        except OSError:
            return False

    # External ``models`` and ``utils`` directories may be namespace packages.
    # A regular package from this notebook's ``code`` directory would otherwise
    # win module discovery even though the selected source root is inserted first.
    sys.path[:] = [code_dir_text] + [
        entry for entry in previous_sys_path
        if not is_local_code_entry(entry) and entry != code_dir_text
    ]
...
def load_legacy_policy_pair(config: Mapping[str, Any], legacy_root: str | Path) -> tuple[Any, Any]:
    """Backward-compatible alias for older notebooks."""

    return load_policy_pair(config, legacy_root)
...
def compute_escape_alignment(
    ...
    """Compute prey-only escape alignment from headings.
...
def compute_dos(geometry: Mapping[str, torch.Tensor], *, d_source: float) -> dict[str, torch.Tensor]:
    """Degree of Swarm: mean prey NND divided by arena maximum distance."""
...
def compute_doa(geometry: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Local nearest-neighbor Degree of Alignment."""
...
def compute_modular_network_maps(
    ...
    *,
    ...
    """Compute vectorized PIN/attention maps for the experiment notebook.
...
__all__ = [
    ...
    "load_policy_pair", "load_legacy_policy_pair",
...
def set_paper_style() -> None:
    """Apply the exact style used by the retained source boxplots."""
...
def _draw_boxplots(
    values_by_case: Mapping[tuple[int, str, str], Any],
    category: str,
    *,
    preserve_tensor_dtype: bool = False,
) -> Any:
    """Render the unchanged two-panel paper boxplot grammar."""
    ...
    ordered_groups = tuple(
        pair for n_prey in (16, 32)
        for pair in ((n_prey, "expert"), (n_prey, "imitation"))
    )
    for metric_index, (ax, metric) in enumerate(
        zip(axes.flat, CATEGORY_METRICS[category])
    ):
        for group_index, (n_prey, kind) in enumerate(ordered_groups):
            raw_values = torch.as_tensor(
                values_by_case[(n_prey, kind, metric)], device="cpu"
            ).flatten()
            raw_values = raw_values[torch.isfinite(raw_values)]
            values = (
                raw_values.numpy()
                if preserve_tensor_dtype
                else raw_values.to(torch.float64).numpy()
            )
...
# ---------------------------------------------------------------------------
# Analysis 1: 20-step large independent Couzin approach cohort


@dataclass(frozen=True)
class CouzinVariant:
    """The training-matched Couzin/GAIL evaluation condition."""

    name: str = "training_matched"
    label: str = "Training-matched evaluation"
    dt: float = 0.25
    alpha: float = 0.01
    theta_dot_max: float = 0.5
...
TRAINING_MATCHED = CouzinVariant()
COUZIN_CHECKPOINT_DIR = CHECKPOINT_ROOT / "couzin"


def _load_policy_class() -> Any:
    config = {"root": CODE_ROOT, "code_subdir": "."}
    _, policy_class, _ = core._load_policy_source_modules(config, CODE_ROOT)
    return policy_class


def _load_couzin_policy_pair() -> tuple[Any, Any]:
    ...
    prey_state = torch.load(
        COUZIN_CHECKPOINT_DIR / "prey_policy_stage1.pth",
        map_location="cpu",
        weights_only=True,
    )
    pred_state = torch.load(
        COUZIN_CHECKPOINT_DIR / "pred_policy_stage1.pth",
        map_location="cpu",
        weights_only=True,
    )
...
def _load_couzin_module() -> Any:
    """Load the local simulator without retaining its generic ``utils`` name."""
    ...
    sys.path[:] = [root_text] + [
        entry for entry in previous_sys_path
        if Path(entry or Path.cwd()).resolve() != local_code_dir
        and entry != root_text
    ]
...
def _generate_couzin_cohort_sources(
    spec: CouzinVariant,
    *,
    n_prey: int,
    ...
    """Generate the source rollouts exactly as in ``couzin_test.py``."""

    couzin = _load_couzin_module()
    records: list[dict[str, Any]] = []
    for replicate_seed in rollout_seeds:
        simulation_seed = int(replicate_seed)
        ...
        try:
            _, _, metrics_list, _, _ = couzin.run_couzin_simulation(
                visualization="off",
                n=n_prey,
                max_steps=spec.rollout_steps,
                number_of_sharks=1,
                area_width=area_width,
                area_height=area_height,
                dt=spec.dt,
                alpha=spec.alpha,
                theta_dot_max=spec.theta_dot_max,
        ...
        clip_id = f"seed_{replicate_seed}_rollout_0000"
        for step, metrics in enumerate(metrics_list):
            xs = np.asarray(metrics["xs"], dtype=float) * area_width
            ys = np.asarray(metrics["ys"], dtype=float) * area_height
            vxs = np.asarray(metrics["vxs"], dtype=float)
            ...
            for agent in range(n_prey + 1):
                ...
                records.append({
                    ...
                    "x": xs[agent],
                    "y": ys[agent],
                    "heading": headings[agent],
                    "vx": vxs[agent],
                    "vy": vys[agent],
                    "step_duration": spec.dt,
                    "sampling_stride": 1,
                    "rollout_seed": replicate_seed,
                    "simulation_seed": simulation_seed,
                    "wall_mode": spec.wall_mode,
                })
    return core.to_canonical_trajectory(records)


def _initial_states(table: Any, n_prey: int) -> tuple[np.ndarray, list[str]]:
    states, clip_ids = [], []
    for indices in core.frame_groups(table):
        clip_id = str(table["clip_id"][indices[0]])
        if clip_id in clip_ids:
            continue
        roles = table["role"][indices].astype(str)
        predator = indices[roles == "predator"]
        prey = indices[roles == "prey"]
        if len(predator) != 1 or len(prey) != n_prey:
            continue
        prey = prey[np.argsort(table["agent_id"][prey].astype(str))]
        ordered = np.concatenate((predator, prey))
        states.append(np.column_stack((
            table["x"][ordered],
            table["y"][ordered],
            table["heading"][ordered],
        )))
...
def _policy_states(
    ...
    *,
    ...
    velocity = torch.stack((torch.cos(heading), torch.sin(heading)), dim=-1)
    velocity *= speed[..., None]
    ...
    predator, prey = neighbors[:, :1], neighbors[:, 1:]
    active = torch.ones((batch, n_agents, n_agents - 1, 1), dtype=position.dtype)
    predator = torch.cat((predator, active[:, :1]), dim=-1)
    predator_flag = torch.zeros(
        (batch, n_prey, n_agents - 1, 1), dtype=position.dtype
    )
    predator_flag[:, :, 0] = 1.0
    prey = torch.cat((predator_flag, prey, active[:, 1:]), dim=-1)
    return predator, prey


def _generate_matched_policy_rollouts(
    spec: CouzinVariant,
    *,
    n_prey: int,
    ...
    prey_policy, pred_policy = _load_couzin_policy_pair()
    ...
    speed = torch.full((batch, n_agents), 5.0, dtype=torch.float32)
    ...
    for step in range(spec.rollout_steps):
        ...
        for rollout in range(batch):
            for agent in range(n_agents):
                ...
                records.append({
                    "source": "policy",
                    "condition": "approach",
                    "clip_id": str(clip_ids[rollout]),
                    "frame": step,
                    "time_step": step,
                    "agent_id": f"{role}_{agent if agent == 0 else agent - 1}",
                    "role": role,
                    "x": float(position_np[rollout, agent, 0]),
                    "y": float(position_np[rollout, agent, 1]),
                    "heading": float(heading_np[rollout, agent]),
                    "step_duration": spec.dt,
                    "sampling_stride": 1,
                    "rollout_seed": seed,
                    "simulation_seed": seed + rollout,
                    "wall_mode": spec.wall_mode,
                    "policy_id": spec.checkpoint,
                })
        ...
        with torch.inference_mode():
            pred_action, _ = pred_policy(
                pred_state.reshape(batch, n_prey, 5), deterministic=False
            )
            prey_action, _ = prey_policy(
                prey_state.reshape(batch * n_prey, n_prey, 6), deterministic=False
            )
        action = torch.cat(
        ...
        velocity *= speed[..., None]
        for rollout in range(batch):
            _reflect_agents(position_np[rollout], heading_np[rollout], 50.0, 50.0)
        velocity = torch.stack((torch.cos(heading), torch.sin(heading)), dim=-1)
        velocity *= speed[..., None]
...
def _extract_approach_windows(
    ...
    *,
    ...
    bundle = core.trajectory_table_to_metric_segments(
        table, expected_n_prey=n_prey, device="cpu", dtype=torch.float32
    )
    selections: list[dict[str, Any]] = []
    for clip in bundle["frame_clips"]:
        geometry = core.prepare_geometry_cache(
            clip, d_source=math.hypot(50.0, 50.0), device="cpu", dtype=torch.float32
        )
        _, distance = core.compute_predator_nearest_distance(geometry)
        detected = core.detect_approach_events(
            distance,
            threshold=0.15,
            closing_lag=3,
            exit_threshold=0.165,
            min_duration=2,
            cooldown_steps=20,
            target_index=geometry["nearest_prey_index"],
            require_target_persistence=True,
        )
        candidates = [
            int(index) for index in detected
            if int(index) >= min_event_step and int(index) + window_steps <= len(distance)
        ]
        if candidates:
            index = candidates[0]
            selections.append({
                "source_clip_id": str(clip["clip_id"]),
                "window_id": f"{clip['clip_id']}_approach_00",
                "start_time": float(clip["time_steps"][index]),
                "stop_time": float(clip["time_steps"][index + window_steps - 1]),
    ...
    windows = core.TrajectoryTable({
        name: np.concatenate(values) for name, values in parts.items()
    })
    return windows, pd.DataFrame(selections)


def _select_clips(table: Any, clip_ids: Sequence[str]) -> Any:
    wanted = {str(value) for value in clip_ids}
    mask = np.asarray(
        [str(value) in wanted for value in table["clip_id"]], dtype=bool
    )
    return core.TrajectoryTable({
        name: values[mask].copy() for name, values in table.columns.items()
    })


def _truncate_windows(table: Any, window_steps: int) -> Any:
...
def _analyze_couzin_table(table: Any, n_prey: int) -> Mapping[str, Any]:
    return core.analyze_case(
        table,
        d_source=math.hypot(50.0, 50.0),
        risk_bin_edges=torch.linspace(0.0144023038, 0.2365715653, 7),
        expected_n_prey=n_prey,
        device="cpu",
        dtype=torch.float32,
        approach_distance_threshold=0.15,
        approach_exit_threshold=0.165,
        approach_closing_lag=3,
        approach_min_duration=2,
        approach_cooldown_steps=20,
        closing_speed_lag=3,
        response_threshold=0.10,
        response_window_steps=20,
        response_lag=1,
        min_bin_count=30,
        min_cluster_count=2,
        n_bootstrap=0,
        step_duration=TRAINING_MATCHED.dt,
    )


def run_couzin_20_step_analysis(
    *,
    ...
    base_seed: int = 302027,
) -> dict[str, Any]:
    """Reproduce the original 20-step result from the 80-step source cohort.

    Requiring a complete 80-step future is intentional: the source notebook
    selected one shared 20/40/80-step cohort before plotting the 20-step view.
    Reducing the selection horizon to 20 would change the observations.
    """

    result: dict[str, Any] = {"groups": {}, "audit": []}
    source_spec = replace(TRAINING_MATCHED, rollout_steps=source_steps)
    maximum_horizon = 80
    for n_prey in (16, 32):
        accepted_windows: list[Any] = []
        accepted_audits: list[pd.DataFrame] = []
        ...
        while accepted < target_windows and attempted < max_source_rollouts:
            ...
            seeds = tuple(
                base_seed + n_prey * 100_000 + attempted + offset
                for offset in range(count)
            )
            batch = _generate_couzin_cohort_sources(
                source_spec, n_prey=n_prey, rollout_seeds=seeds
            )
            batch_ids = list(dict.fromkeys(batch["clip_id"].astype(str)))
            try:
                windows, audit = _extract_approach_windows(
                    batch,
                    n_prey=n_prey,
                    window_steps=maximum_horizon,
                    min_event_step=burn_in_steps,
                )
            except ValueError:
                windows, audit = None, pd.DataFrame()
            accepted_ids = (
                set() if audit.empty else set(audit["source_clip_id"].astype(str))
            )
            remaining = target_windows - accepted
            chosen_ids = (
                list(dict.fromkeys(audit["source_clip_id"].astype(str)))[:remaining]
                if not audit.empty else []
            )
            chosen = set(chosen_ids)
            if chosen_ids:
                chosen_audit = audit[
                    audit["source_clip_id"].astype(str).isin(chosen)
                ].copy()
                accepted_windows.append(
                    _select_clips(windows, chosen_audit["window_id"].astype(str))
                )
                accepted_audits.append(chosen_audit)
                accepted += len(chosen_ids)
            audit_by_source = (
                audit.set_index("source_clip_id").to_dict("index")
                if not audit.empty else {}
            )
        ...
        if not accepted_windows:
            raise ValueError(f"No valid N={n_prey} approach events found")
        expert_80 = core.concatenate_trajectory_tables(accepted_windows)
        event_audit = pd.concat(accepted_audits, ignore_index=True)
        starts, clip_ids = _initial_states(expert_80, n_prey)
        imitation_80 = _generate_matched_policy_rollouts(
            replace(TRAINING_MATCHED, rollout_steps=maximum_horizon),
            n_prey=n_prey,
            initial_states=starts,
            clip_ids=clip_ids,
            seed=base_seed + 900_000 + n_prey,
        )
        expert = _truncate_windows(expert_80, 20)
        imitation = _truncate_windows(imitation_80, 20)
        result["groups"][n_prey] = {
            "expert_analysis": _analyze_couzin_table(expert, n_prey),
            "imitation_analysis": _analyze_couzin_table(imitation, n_prey),
    ...
    result["audit"] = pd.DataFrame(result["audit"])
    return result


def plot_couzin_boxplots(result: Mapping[str, Any], category: str) -> Any:
    values = {
        (n_prey, kind, metric): result["groups"][n_prey][f"{kind}_analysis"]
        ["per_clip"][metric]
        for n_prey in (16, 32)
        for kind in ("expert", "imitation")
        for metric in METRIC_SPECS
    }
    return _draw_boxplots(values, category, preserve_tensor_dtype=True)


# ---------------------------------------------------------------------------
# Analysis 2: biological data versus biological GAIL imitation

BIO_POLICY = {
    "root": CODE_ROOT,
    "code_subdir": ".",
    "prey_checkpoint": CHECKPOINT_ROOT / "biological/prey_policy_stage1.pth",
    "pred_checkpoint": CHECKPOINT_ROOT / "biological/pred_policy_stage1.pth",
    "prey_features": 6,
    "pred_features": 5,
    "deterministic": False,
    "environment": {
...
def run_biological_gail_analysis(
    *,
    rollout_seeds: Sequence[int] = (2027, 12027, 22027),
    n_rollouts_per_seed: int = 10,
    rollout_steps: int = 1000,
    device: str | torch.device | None = None,
) -> dict[str, Any]:
    """Compute only the four per-clip metrics used by the Stage-2 boxplots."""
    ...
    bio_window_root = (
        DATA_ROOT
        / "1. Data Processing/Processed/video/expert_tensors/windows"
    )
    bio_expert_root_32 = (
        DATA_ROOT
        / "1. Data Processing/Processed/video/expert_tensors/"
        "32_prey_interactions/2. filtered_frames"
    )
    init_pool_partitions = {
        16: {"interaction": (0, 1619), "attack": (1619, 3293)},
    ...
    for n_prey in (16, 32):
        path = (
            DATA_ROOT
            / f"1. Data Processing/Processed/init_pool/init_pool_{n_prey}prey.pt"
        )
        pool = _load_tensor(path)
        start, stop = init_pool_partitions[n_prey]["interaction"]
        interaction_pools[n_prey] = pool[start:stop].clone()

    cases: dict[str, Any] = {}
    cases["bio_expert_16"] = core.load_biological_window_trajectories(
        bio_window_root,
        n_prey=16,
        condition="interaction",
        coordinate_scale=2160,
        effective_fps=effective_fps,
        sampling_stride=2,
    )
    expert_32 = core.load_expert_trajectories(
        bio_expert_root_32,
        n_prey=32,
        condition="interaction",
        image_y_down=True,
        coordinate_height=2160,
        role_map={"1": "predator", "2": "prey"},
        require_complete_frames=True,
        fps=effective_fps,
    )
    cases["bio_expert_32"] = core.with_constant_metadata(
        expert_32,
        fps=effective_fps,
        step_duration=1.0 / effective_fps,
        sampling_stride=2,
    )
    for n_prey in (16, 32):
        cases[f"bio_imitation_{n_prey}"] = core.generate_policy_rollouts(
            BIO_POLICY,
            CODE_ROOT,
            n_prey=n_prey,
            n_rollouts=n_rollouts_per_seed,
            rollout_steps=rollout_steps,
            seed=int(rollout_seeds[0]),
            rollout_seeds=tuple(int(value) for value in rollout_seeds),
            init_pool_tensor=interaction_pools[n_prey],
            condition="interaction",
            effective_fps=effective_fps,
            sampling_stride=2,
    ...
    analyses = {}
    with torch.inference_mode():
        for name, table in tqdm(cases.items(), desc="Biological paper metrics"):
            n_prey = 16 if name.endswith("16") else 32
            analyses[name] = core.analyze_case(
                table,
                d_source=math.hypot(2160, 2160),
                risk_bin_edges=risk_edges,
                expected_n_prey=n_prey,
                device=target_device,
                dtype=torch.float32,
                ...
                n_bootstrap=0,
                ci_level=0.95,
                bootstrap_seed=int(rollout_seeds[0]),
                step_duration=1.0 / effective_fps,
            )
    per_clip = {
        name: {
            metric: _finite_1d(result["per_clip"][metric])
            for metric in METRIC_SPECS
        }
        for name, result in analyses.items()
    }
    return {"per_clip": per_clip, "analyses": analyses}


def plot_biological_boxplots(result: Mapping[str, Any], category: str) -> Any:
    values = {
        (n_prey, kind, metric): result["per_clip"]
        [f"bio_{kind}_{n_prey}"][metric]
        for n_prey in (16, 32)
        for kind in ("expert", "imitation")
        for metric in METRIC_SPECS
    }
    return _draw_boxplots(values, category)


# ---------------------------------------------------------------------------
# Analysis 3: Couzin-based data and biological-recording time-step grids

SIMULATION_TIMESTEPS = (1, 10, 30, 100, 250, 500)


def _generate_simulation_couzin_data(rollout_steps: int, seed: int) -> Any:
    """Match the legacy Couzin source used by ``simulation_plots.ipynb``."""
    ...
    records: list[dict[str, Any]] = []
    for step, metrics in enumerate(metrics_list):
        xs = np.asarray(metrics["xs"], dtype=float) * 50.0
        ys = np.asarray(metrics["ys"], dtype=float) * 50.0
        vxs = np.asarray(metrics["vxs"], dtype=float)
        vys = np.asarray(metrics["vys"], dtype=float)
        headings = np.arctan2(vys, vxs)
        for agent in range(33):
            role = "predator" if agent == 0 else "prey"
            records.append({
                "source": "couzin_expert",
                "condition": "approach",
                "clip_id": "rollout_0000",
                "frame": step,
                "time_step": step,
                "agent_id": f"{role}_{agent if agent == 0 else agent - 1}",
                "role": role,
...
def _render_rollout_video(
    ...
    *,
    fps: float = 30.0,
    figure_pixels: int = 720,
) -> Path:
    import cv2

    times = np.asarray(table["time_step"], dtype=int)
    roles = np.asarray(table["role"]).astype(str)
    xs = np.asarray(table["x"], dtype=float)
    ys = np.asarray(table["y"], dtype=float)
    headings = np.asarray(table["heading"], dtype=float)
    frame_numbers = np.unique(times)
    dpi = 120
    fig, ax = plt.subplots(figsize=(figure_pixels / dpi, figure_pixels / dpi), dpi=dpi)
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.08, top=0.98)
    ax.set_xlim(0, 50)
    ax.set_ylim(0, 50)
    ax.set_aspect("equal")
    arrow_length = 1.75
    prey_zeros = np.zeros(32)
    prey_quiver = ax.quiver(
        prey_zeros,
        prey_zeros,
        prey_zeros,
        prey_zeros,
    ...
    try:
        for time_step in frame_numbers:
            idx = np.flatnonzero(times == time_step)
            prey_idx = idx[roles[idx] == "prey"]
            pred_idx = idx[roles[idx] == "predator"]
            prey_quiver.set_offsets(np.column_stack((xs[prey_idx], ys[prey_idx])))
            prey_quiver.set_UVC(
                arrow_length * np.cos(headings[prey_idx]),
                arrow_length * np.sin(headings[prey_idx]),
            )
            pred_quiver.set_offsets(np.column_stack((xs[pred_idx], ys[pred_idx])))
            pred_quiver.set_UVC(
                arrow_length * np.cos(headings[pred_idx]),
                arrow_length * np.sin(headings[pred_idx]),
            )
            fig.canvas.draw()
...
def _plot_video_timesteps(
    ...
    *,
    ...
    requested = np.asarray(tuple(timesteps), dtype=int)
    if (requested >= count).any():
        capture.release()
        raise IndexError("Timestep exceeds video length")
    fig, axes = plt.subplots(2, 3, figsize=figsize)
    try:
        for ax, timestep in zip(axes.flat, requested):
            capture.set(cv2.CAP_PROP_POS_FRAMES, int(timestep))
            ok, image = capture.read()
            if not ok:
                raise RuntimeError(f"Could not read frame {timestep}")
...
def _extract_video_plot_area(image: np.ndarray) -> tuple[np.ndarray, bool]:
    rgb = np.asarray(image)[..., :3].copy()
    dark = np.min(rgb, axis=2) < 100
    content_dark = dark.copy()
    edge_guard = max(4, int(min(rgb.shape[:2]) * 0.006))
    content_dark[:edge_guard, :] = False
    content_dark[-edge_guard:, :] = False
    content_dark[:, :edge_guard] = False
    content_dark[:, -edge_guard:] = False
    rows = np.flatnonzero(content_dark.sum(axis=1) >= int(rgb.shape[1] * 0.50))
    columns = np.flatnonzero(content_dark.sum(axis=0) >= int(rgb.shape[0] * 0.50))
    if len(rows) < 2 or len(columns) < 2:
        return rgb, False
    top, bottom = int(rows[0]), int(rows[-1])
    left, right = int(columns[0]), int(columns[-1])
    if bottom - top < rgb.shape[0] * 0.5 or right - left < rgb.shape[1] * 0.5:
        return rgb, False
    plot_area = rgb[top + 1:bottom, left + 1:right]
    plot_area[np.min(plot_area, axis=2) >= 245] = 255
    return plot_area, True


def _make_grid_paper_ready(
    figure: Any,
    axes: np.ndarray,
    *,
    panel_title_fontsize: float = 16,
) -> None:
    for ax in axes.flat:
        image = ax.images[0]
        plot_area, has_coordinate_axes = _extract_video_plot_area(
            np.asarray(image.get_array())
        )
        image.set_data(plot_area)
        image.set_interpolation("none")
        if has_coordinate_axes:
            image.set_extent((0, 50, 0, 50))
            ax.axis("on")
            ax.set_xlim(0, 50)
            ax.set_ylim(0, 50)
            ...
            ax.axis("off")
        ax.set_title(
            ax.get_title(), fontsize=panel_title_fontsize, fontweight="medium", pad=2
        )
        ax.set_anchor("N")
...
def _evenly_spaced_video_frames(video_path: Path, count: int = 6) -> tuple[int, ...]:
    import cv2
    ...
    try:
        ...
        capture.release()
    return tuple(np.rint(np.linspace(0, frame_count - 1, count)).astype(int))


def _create_selected_simulation_plots(
    *,
    timesteps: Sequence[int] = SIMULATION_TIMESTEPS,
    couzin_data_seed: int = 3121,
    panel_title_fontsize: float = 16,
) -> OrderedDict[str, tuple[Any, np.ndarray]]:
    """Create only the two time-step grids retained in the paper notebook."""

    requested = tuple(int(value) for value in timesteps)
    if len(requested) != 6 or min(requested) < 0:
        raise ValueError("Exactly six non-negative timesteps are required")
    recording_path = VIDEO_DIR / "real_bio_recording.mp4"
    if not recording_path.is_file():
        raise FileNotFoundError(f"Missing biological recording: {recording_path}")
    figures: OrderedDict[str, tuple[Any, np.ndarray]] = OrderedDict()
    with tempfile.TemporaryDirectory(prefix="icra_paper_analysis_") as temp_dir:
        couzin_video = Path(temp_dir) / "simulation_couzin_data.mp4"
        table = _generate_simulation_couzin_data(
            max(requested) + 1, couzin_data_seed
        )
        _render_rollout_video(table, couzin_video)
        figure, axes = _plot_video_timesteps(couzin_video, requested)
        _make_grid_paper_ready(
            figure, axes, panel_title_fontsize=panel_title_fontsize
        )
        figures["couzin_data"] = (figure, axes)

        recording_steps = _evenly_spaced_video_frames(recording_path)
        figure, axes = _plot_video_timesteps(recording_path, recording_steps)
        _make_grid_paper_ready(
            figure, axes, panel_title_fontsize=panel_title_fontsize
        )
        figures["biological_recording"] = (figure, axes)
    return figures


def create_selected_simulation_plots(
    *,
    timesteps: Sequence[int] = SIMULATION_TIMESTEPS,
    couzin_data_seed: int = 3121,
    panel_title_fontsize: float = 16,
) -> OrderedDict[str, tuple[Any, np.ndarray]]:
    """Create the two grids with the fresh-kernel style of the source notebook."""

    # Both preceding boxplot sections intentionally set paper-wide rcParams.
    # The original simulation notebook, however, ran in a fresh kernel using
    # Matplotlib defaults.  Isolating that state keeps a complete top-to-bottom
    # rerun pixel-identical to the original grids.
    with plt.rc_context(rc=plt.rcParamsDefault):
        return _create_selected_simulation_plots(
            timesteps=timesteps,
            couzin_data_seed=couzin_data_seed,
...
__all__ = [
    "TRAINING_MATCHED",
    "create_selected_simulation_plots",
    "plot_biological_boxplots",
    "plot_couzin_boxplots",
    "run_biological_gail_analysis",
    "run_couzin_20_step_analysis",
]
